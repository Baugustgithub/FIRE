...
def normalize_return(r):
    ...
    - If r > 100, assume it's accidentally a percent with extra scale; divide by 100
    - Clamp to [-90%, +200%] to keep charts sane (still permissive)
    """
    if r is None:
        return 0.0
    try:
...
if granular_mode:
    st.sidebar.caption("Specify starting balances & expected returns for any accounts you want. Anything unassigned goes to 'Other Investments' using the global return.")
...
if st.sidebar.button("🚀 Run FIRE Simulation"):
    ...
    years_to_retirement = max(1, int(round(retirement_age)))  # treat as years from now, min 1
...
""")
    ...
    # -----------------------------
    # Snapshot selector (FI vs Ret) — crash-proof
    # -----------------------------
